"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.ProfesorController = void 0;
const common_1 = require("@nestjs/common");
const profesor_service_1 = require("./profesor.service");
const generic_controller_1 = require("../generic/generic.controller");
const login_dto_1 = require("./dto/login.dto");
let ProfesorController = class ProfesorController extends generic_controller_1.GenericController {
    constructor(profesorService) {
        super(profesorService);
        this.profesorService = profesorService;
    }
    async login(loginDto) {
        return this.profesorService.login(loginDto);
    }
    async getCsvData() {
        const filePath = 'src/profesor/files/alumnos.csv';
        try {
            const entities = await this.profesorService.parseCsvToEntities(filePath);
            return entities;
        }
        catch (error) {
            throw new Error('Error al leer el archivo CSV.');
        }
    }
};
__decorate([
    (0, common_1.Post)('login'),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [login_dto_1.LoginDto]),
    __metadata("design:returntype", Promise)
], ProfesorController.prototype, "login", null);
__decorate([
    (0, common_1.Get)('/importaralumnos'),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", Promise)
], ProfesorController.prototype, "getCsvData", null);
ProfesorController = __decorate([
    (0, common_1.Controller)('profesores'),
    __metadata("design:paramtypes", [profesor_service_1.ProfesorService])
], ProfesorController);
exports.ProfesorController = ProfesorController;
//# sourceMappingURL=profesor.controller.js.map